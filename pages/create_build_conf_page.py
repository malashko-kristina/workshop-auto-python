...
class MenuListCreateBuildConfFragment(BasePage):
    def __init__(self, page):
        self.page = page
        super().__init__(page)
        self.create_build_conf_from_url_selector = ("a.createOption:"
                                                    "has-text('From a repository URL')")
        self.create_build_conf_manually_selector = ("a.createOption:"
                                                    "has-text(' Manually')")
...
class CreateBuildConfFormContainerFragment(BasePage):
    def __init__(self, page):
        ...
        self.create_build_conf_button = ('input.btn.btn_primary'
                                         '.submitButton[name="createBuildType"]')
    ...
    def check_error_message_empty_build_name(self):
        with allure.step("Проверка текста ошибки о пустом имени билда"):
            ...
            self.actions.assert_text_in_element(self.build_name_error,
    ...
    def check_error_message_empty_build_id(self):
        with allure.step("Проверка текста ошибки о пустом id билда"):
            ...
            self.actions.assert_text_in_element(self.build_id_error,
    ...
    def check_error_message_invalid_build_id(self, build_conf_id, first_symbol):
        with allure.step("Проверка текста ошибки о пустом невалидном id билда"):
            ...
            self.actions.assert_text_in_element(self.build_id_error,
                                                ...
                                                f' "{build_conf_id}" is invalid: starts'
                                                f' with non-letter character'
                                                f' \'{first_symbol}\'. ID should start'
                                                f' with a latin letter and contain only'
                                                f' latin letters, digits and underscores'
                                                f' (at most 225 characters).')
            self.actions.check_error_text_color(self.build_id_error)
    ...
    def check_error_message_used_build_name(self, build_conf_name, project_name):
        with allure.step("Проверка текста ошибки о уже используемом имени билда"):
            self.actions.wait_for_selector(self.build_name_error)
            self.actions.assert_text_in_element(self.build_name_error,
                                                f'Build configuration with name'
                                                f' "{build_conf_name}" already'
                                                f' exists in project: "{project_name}"')
            self.actions.check_error_text_color(self.build_name_error)
...
class BuildConfCreationPage(BasePage):
    def __init__(self, page, project_id):
        super().__init__(page)
        self.page_url = (f'/admin/createObjectMenu.html?projectId={project_id}'
                         f'&showMode=createBuildTypeMenu&cameFromUrl=%2Fadmin%2'
                         f'FeditProject.html%3FprojectId%3DTests')
    ...
    def create_build_conf(self, build_conf_id, build_conf_name, description):
        ...
        with allure.step("Добавления информации для создания билд конфигурации"):
            self.create_build_conf_form_container.input_build_conf_details(
    ...
    def check_url_after_build_create(self, build_conf_id, project_id):
            self.page_url = (f'/admin/editVcsRoot.html?action=addVcsRoot&editingScope'
    ...
    def check_error_empty_build_name(self):
        with allure.step("Проверка ошибки создания билда из-за пустого поля имени"):
                self.create_build_conf_form_container.check_error_message_empty_build_name()
    def check_error_empty_build_id(self):
        with allure.step("Проверка ошибки создания билда из-за пустого поля id"):
    ...
    def check_error_invalid_build_id(self, build_conf_id, first_symbol):
        with allure.step("Проверка ошибки создания билда из-за invalid buuld id"):
            self.create_build_conf_form_container.check_error_message_invalid_build_id(
                build_conf_id, first_symbol)
    def check_error_used_build_id(self, build_conf_name, project_name):
        with allure.step("Проверка ошибки создания билд из-за уже используемого поля name"):
            self.create_build_conf_form_container.check_error_message_used_build_name(
