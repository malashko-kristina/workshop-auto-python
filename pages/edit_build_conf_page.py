...
class BuildConfCopyFragment(BasePage):
    ...
    def copy_build_conf (self, new_build_conf_id):
        ...
        with allure.step("Клик на кнопку copy для копирования билд конфигурации"):
            self.actions.is_button_active(self.copy_button)
        ...
        with allure.step("Добавления нового билд конф id в поле build cond id"):
            ...
            time.sleep(2)
            self.actions.is_button_active(self.copy_build_button)
            time.sleep(2)
    ...
    def message_after_copy_build_conf(self):
        ...
        time.sleep(2)
        self.actions.assert_text_in_element(self.copy_build_message, f'Build configuration has been copied successfully. Build configuration name has been changed because another build configuration with the same name was found in this project.')
    ...
    def tap_on_cancel_button(self):
        with allure.step("Клик на кнопку отмены"):
            self.actions.is_button_active(self.cancel_button)
...
class BuildConfCopyPage(BasePage):
    ...
    def copy_build_conf(self, new_build_conf_id):
        ...
        with allure.step("Копирование билд конфигурации"):
            self.build_conf_copy.copy_build_conf(new_build_conf_id)
            time.sleep(2)
            self.page_url = f'/admin/editBuild.html?id=buildType:{new_build_conf_id}'
            self.actions.wait_for_url_change(self.page_url)
            time.sleep(2)
        with allure.step("Проверка сообщения лю успешном копировании билд конфигурации"):
...
class BuildConfCopyErrorPage(BasePage):
    ...
    def copy_build_conf_error(self, new_build_conf_id, build_conf_id, first_symbol):
        with allure.step("Переход на страницу редактирования"):
            self.go_to_edit_build_conf_page()
            time.sleep(2)
        with allure.step("Копирование билд конфигурации"):
...
class BuildConfDDeleteFragment(BasePage):
    ...
    def delete_build_conf (self):
        ...
        with allure.step("Клик на кнопку удаления билд конфигурации"):
            self.actions.is_button_active(self.delete_button)
            self.actions.click_button(self.delete_button)
        with allure.step("Клик на кнопку 'Enter' на физической клавиатуре"):
            self.actions.press_on_enter_keyboard_button()
            time.sleep(2)

    def message_after_delete_build_conf(self, build_conf_id):
        ...
        with allure.step("Проверка сообщения об удалении билд конфигурации"):
            self.actions.assert_text_in_element(self.delete_build_conf_message, f'Build configuration "{build_conf_id}" has been removed.Please note that build configuration related data (builds history, artifacts and so on) will be cleaned from the database when next clean-up process is started, see')
...
class BuildConfDeletePage(BasePage):
    ...
    def delete_build_conf(self, project_id, build_conf_id):
        with allure.step("Удаление билд конфигурации"):
            ...
            time.sleep(2)
        with allure.step("Проверка сообщения об удалении билд конфигурации"):
            self.build_conf_delete.message_after_delete_build_conf(build_conf_id)
