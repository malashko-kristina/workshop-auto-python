...
class AgentsPage(BasePage):
    def __init__(self, page):
        ...
        self.unauthorized_agents_selector = (
            "[data-test-agents-sidebar-title]" " >> text='UNAUTHORIZED AGENTS'"
        )
        self.authorize_button_selector = "button[data-test-authorize-agent]"
        self.authorize_button_at_popup_selector = (
            "[data-portaltarget]" " button.ring-button-primary"
        )

    @allure.step(
        "Navigating to the agents page and authorizing a new unauthorized agent"
    )
