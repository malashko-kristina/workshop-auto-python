    ...
    def check_text_in_selector(self, project_name):
        with allure.step("Проверка наличия текста на странице"):
            ...
            self.actions.assert_text_in_element(
                self.message_created_project_locator,
                f'Project "{project_name}" has been successfully created. You can now create a build configuration.',
            )
    ...
    def success_message_copy_project(self):
        with allure.step("Проверка отображения текста об успешном копировании проекта"):
            ...
            self.actions.assert_text_in_element(
                ...
                "Project has been copied successfully. Project name has been changed because another project with the same name already exists. Project-associated settings were copied.",
            )

    def error_message_copy_project_empty_id(self):
        with allure.step(
            "Проверка отображения текста об ошибке копирования проекта (project id пустой)"
        ):
...
class EditProjectPageFragment(BasePage):
    ...
    def check_description(self, description):
        with allure.step("Проверка description проекта"):
            self.actions.assert_element_attribute(
                self.project_description_selector, "value", f"{description}"
            )
    ...
    def warning_message_displaying(self):
        with allure.step(
            "Отображение предупреждения после редактирования данных проекта"
        ):
            self.actions.wait_for_selector(self.warning_message)

    def error_message_invalid_project_id_edit(self, project_id):
        with allure.step(
            "Проверка отображения сообщения об ошибке при редактировании проекта (невалидный project id)"
        ):
            self.actions.wait_for_selector(self.invalid_id_error)
            self.actions.assert_text_in_element(
                self.invalid_id_error,
                f"Project ID \"{project_id}\" is invalid: starts with non-letter character '{project_id[0]}'. ID should start with a latin letter and contain only latin letters, digits and underscores (at most 225 characters).",
            )

    def error_message_empty_project_id_copy(self):
        with allure.step(
            "Проверка отображения сообщения об ошибке при копировании проекта (пустой project id)"
        ):
    ...
    def success_message_edit_saved(self):
        with allure.step(
            "Отображения сообщения об успешном сохранении данных редактирования проекта"
        ):
...
class AddBuildConf(BasePage):
    def __init__(self, page):
        super().__init__(page)
        self.build_conf_add_button_selector = (
            'span.addNew:has-text("Create build configuration")'
        )
        self.build_conf_templates_button_selector = (
            'span.addNew:has-text("Create template")'
        )
...
class EditProjectFormPage(BasePage):
    ...
    def check_success_project_creation(self, name, project_id, description):
        with allure.step(
            "Проверка сообщения об успешном создании проекта и данных проекта"
        ):
    ...
    def check_error_message_project_copy(self):
        with allure.step(
            "Отображение сообщения об ошибке при создании копии проекта с пустым id"
        ):
