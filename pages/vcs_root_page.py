    ...
    def check_text_in_selector(self):
        with allure.step("Проверка наличия текста на странице"):
            self.actions.assert_text_in_element(
                self.message_created_build_conf_locator,
...
class AddVCSFormPage(BasePage):
    def __init__(self, page, build_conf_id, project_id):
        ...
        self.page_url = (f"/admin/editVcsRoot.html?action=addVcsRoot&editingScope"
                         ...
                         f"BuildTypeVcsRoots.html%3Finit%3D1%26id%3DbuildType"
                         f"%3A{build_conf_id}d%26cameFromUrl%3D%252Fadmin%252Fedi"
                         f"tProject.html%253Finit%253D1%2526projectId%253D{project_id}"
                         f"&cameFromTitle=Version%20Control%20Settings&showSkip=true")
    ...
    def skip_vcs(self, build_conf_id, project_id):
        ...
        with allure.step("Проверка загрузки страницы"):
            self.page_url = (f"/admin/editBuildTypeVcsRoots.html?init=1&id="
                             f"buildType:{build_conf_id}&cameFromUrl=%2Fadmin%2"
                             f"FeditProject.html%3Finit%3D1%26projectId%3D{project_id}")
