...
class ProjectCreationPage(BasePage):
    ...
    def check_error_empty_project_name_is_visible(self):
        with allure.step(f"Проверка нахождения текста об ошибке 'Project name is empty' в селекторе {self.create_form_container.error_empty_name}"):
            self.create_form_container.error_empty_project_name()

    def check_error_invalid_project_id_is_visible(self, project_id):
        with allure.step(f"Проверка нахождения текста об ошибке о том, что project id уже используется"):
