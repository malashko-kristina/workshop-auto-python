...
def test_run_build_conf(
    ...
    with allure.step(
        "Переход на страницу для клика на кнопку добавления шагов для билд конфигурации"
    ):
    ...
    with allure.step(
        "Проверка добавления шагов для билд конфигурации c пустым script для command line"
    ):
    ...
    with allure.step(
        "Проверка добавления шагов для билд конфигурации c пустым id step"
    ):
    ...
    with allure.step(
        "Проверка добавления шагов для билд конфигурации c невалидным id step"
    ):
    ...
    with allure.step(
        "Проверка запуска билд конфигурации с некорректно веденным script для command line"
    ):
        add_new_step_error_browser.add_new_build_step(
    ...
    with allure.step(
        "Запуск билд конфигурации с некорректно введенным script для command line"
    ):
