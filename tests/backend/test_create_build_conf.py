...
class TestBuildCreateWithInvalidData:
    ...
    def test_create_build_conf_with_empty_name_field(self, super_admin, user_create, project_data, build_conf_data_with_empty_name):
        ...
        with allure.step("Отправка запроса на создание билд конфигурации с пустым 'name' полем"):
            ...
            build_config_response = super_admin.api_manager.build_conf_api.create_build_conf(
                build_conf_data_1.model_dump(), expected_status=HTTPStatus.BAD_REQUEST)
        with pytest.assume:
    ...
    def test_create_build_conf_with_invalid_project_id(self, super_admin, user_create, project_data, build_conf_data_with_invalid_project_id):
        ...
        with pytest.assume:
            assert f"NotFoundException: No project found by locator 'count:1,id:{
                build_conf_data_1.project['id']}'. Project cannot be found by external id '{build_conf_data_1.project['id']}" in build_config_response.text
...
class TestBuildConfCreateWithAlreadyUsedIdAndName:
    ...
    def test_create_build_conf_when_build_conf_exists_with_name(self, super_admin, user_create, project_data, build_conf_data):
        ...
        with allure.step("Отправка запроса на создание билд конфигурации с не уникальным 'name'"):
            ...
            build_config_response_2 = super_admin.api_manager.build_conf_api.create_build_conf(
                build_conf_data_2.model_dump(), expected_status=HTTPStatus.BAD_REQUEST)
        with pytest.assume:
            assert f'DuplicateBuildTypeNameException: Build configuration with name "{
                build_conf_data_2.name}" already exists in project: "{project_data_1.name}"' in build_config_response_2.text
    ...
    def test_create_build_conf_when_build_conf_exists_with_id(self, super_admin, user_create, project_data, build_conf_data):
        ...
        with allure.step("Отправка запроса на создание билд конфигурации с неуникальным 'id'"):
            ...
            build_config_response_2 = super_admin.api_manager.build_conf_api.create_build_conf(
                build_conf_data_2.model_dump(), expected_status=HTTPStatus.BAD_REQUEST).text
        with pytest.assume:
            assert f'DuplicateExternalIdException: The build configuration / template ID "{
                build_conf_data_2.id}" is already used by another configuration or template' in build_config_response_2
...
class TestBuildConfCopy:
    ...
    def test_project_copy(self, super_admin, user_create, project_data, build_conf_data, build_conf_data_copy):
        ...
        with allure.step("Отправка запроса на создание копии уже существующей билд конфигурации"):
            ...
            copy_build_conf_response = super_admin.api_manager.build_conf_api.create_build_conf_copy(
                build_conf_data_copy_1.model_dump(), project_data_1.id).text
        ...
        with pytest.assume:
            assert build_conf_model_response.id == build_conf_data_copy_1.id, \
                f"expected project id= {build_conf_data_copy_1.id}, but '{
                    build_conf_model_response.id}' given"
        with pytest.assume:
    ...
    def test_build_conf_copy_with_invalid_source_build_conf(self, super_admin, user_create, project_data, build_conf_data, build_conf_data_copy_with_invalid_parent_build_conf):
        ...
        with allure.step("Отправка запроса на создание копии уже существующей билд конфигурации с неизвестным source build conf"):
            ...
            copy_build_conf_response = super_admin.api_manager.build_conf_api.create_build_conf_copy(
                build_conf_data_copy_1.model_dump(), project_data_1.id, expected_status=HTTPStatus.NOT_FOUND)
        with pytest.assume:
            assert f"NotFoundException: No build type or template is found by id, internal id or name '{
                build_conf_data_copy_1.sourceBuildTypeLocator}'" in copy_build_conf_response.text
...
class TestBuildConfCreateDeleteAndGetInfo:
    ...
    def test_create_build_conf_delete_and_get_info(self, super_admin, user_create, project_data, build_conf_data_without_deleting_id):
        ...
        with allure.step("Отправка запроса на удаление билд конфигурации"):
            build_conf_delete_response = super_admin.api_manager.build_conf_api.delete_build_conf(
                build_conf_model_response.id)
        with pytest.assume:
        ...
        with allure.step("Отправка запроса на получение информации об удаленной билд конфигурации"):
            get_about_delete_build_conf_response = super_admin.api_manager.build_conf_api.get_build_conf(
                build_conf_model_response.id, expected_status=HTTPStatus.NOT_FOUND)
        with pytest.assume:
            assert f"NotFoundException: No build type nor template is found by id '{
                build_conf_data_1.id}'" in get_about_delete_build_conf_response.text
